codeunit 50104 "Match Bank Rec. Line"
{
    ...
    begin
        if SelectedBankAccountLedgerEntry.FindFirst() then begin
            BankAccReconciliationLine.get(SelectedBankAccReconciliationLine."Statement Type");
    ...
    begin
        if SelectedBankAccReconciliationLine.FindSet() then
            repeat
                ...
                //RehanSayed
                // BankAccountLedgerEntry.SetRange("UTR No.", BankAccReconciliationLine."UTR No");
    ...
    begin
        ...
        if not BankAccReconciliationLine.IsEmpty() then begin
            ...
            if Overwrite then begin
                ...
                // BankAccReconciliationLine.SetRange("UTR No",BankAccReconciliation.);
                //BankAccReconciliationLine.SetFilter("NEFT No.", BankAccountLedgerEntry."NEFT/RTGS");
                RemoveMatchesFromRecLines(BankAccReconciliationLine);
            end;
        end;
        if GuiAllowed() then ProgressDialog.Open(ProgressBarMsg);
        // Lines to match
        BankAccReconciliationLine.Reset();
        BankAccReconciliationLine.FilterBankRecLines(BankAccReconciliation, Overwrite);
        // Candidate Bank Account Ledger Entries
        //Rehan //BankAccountLedgerEntry.SetBankReconciliationCandidatesFilters(BankAccReconciliation);
        ...
        if not TempBankAccLedgerEntryMatchingBuffer.IsEmpty() then
            if BankAccReconciliationLine.FindSet() then
                repeat
                    BankAccountLedgerEntry.SetRange("Bank Account No.", BankAccReconciliationLine."Bank Account No.");
                    BankAccountLedgerEntry.SetRange("UTR No.", BankAccReconciliationLine."UTR No");
                    BankAccountLedgerEntry.SetRange("Entry No.", BankAccReconciliationLine."Entry No");

                    if (BankAccountLedgerEntry."Entry No." = BankAccReconciliationLine."Entry No") or
                   (BankAccountLedgerEntry."UTR No." = BankAccReconciliationLine."UTR No") then
                        if BankAccountLedgerEntry.FindFirst() then begin
                            // If there are no candidate Bank Account Ledger Entries left, we can stop as there will be no other match
                            if TempBankAccLedgerEntryMatchingBuffer.IsEmpty() then break;
        ...
        if GuiAllowed() then ProgressDialog.Close();
    ...
    begin
        ...
        if not TempBankAccLedgerEntryMatchingBuffer.FindSet() then exit;
        repeat // Date assumptions are only considered when Bank Rec. Line has Transaction Date <> 0
            ...
            if TransactionTooEarly then TempBankAccLedgerEntryMatchingBuffer.Delete();
            if (not TransactionTooEarly) and (not TransactionTooLate) then begin
                ...
                if MatchingIsAcceptable(BankAccReconciliationLine, TempBankAccLedgerEntryMatchingBuffer, TempMatchingDetailsBankStatementMatchingBuffer) then begin
                    ...
                    if MatchIsBetter(TempMatchingDetailsBankStatementMatchingBuffer, TempBestBankStatementMatchingBuffer) then TempBestBankStatementMatchingBuffer.Copy(TempMatchingDetailsBankStatementMatchingBuffer);
                end
            end;
        ...
        if TempBestBankStatementMatchingBuffer."Entry No." <> 0 then begin
            ...
            TempBankStatementMatchingBuffer.Insert();
            if TempBankAccLedgerEntryMatchingBuffer.Get(TempBestBankStatementMatchingBuffer."Entry No.", TempBankAccLedgerEntryMatchingBuffer."Account Type"::"Bank Account") then TempBankAccLedgerEntryMatchingBuffer.Delete();
        end;
    end;
    ...
    begin
        if TempToCompareBankStatementMatchingBuffer."Entry No." = 0 then exit(true);
        ...
        if AmountDifferenceSmaller then exit(true);
        if abs(TempBankStatementMatchingBuffer."Amount Difference") > abs(TempToCompareBankStatementMatchingBuffer."Amount Difference") then exit(false);
        if TextMatchingBetter then exit(true);
        if GetMaxTextScore(TempBankStatementMatchingBuffer) < GetMaxTextScore(TempToCompareBankStatementMatchingBuffer) then exit(false);
    ...
    begin
        ...
        if MaxTextScore < TempBankStatementMatchingBuffer."Ext. Doc. No. Score" then MaxTextScore := TempBankStatementMatchingBuffer."Ext. Doc. No. Score";
        if MaxTextScore < TempBankStatementMatchingBuffer."Description Score" then MaxTextScore := TempBankStatementMatchingBuffer."Description Score";
        exit(MaxTextScore);
    end;
    ...
    begin
        exit((TempMatchingDetailsBankStatementMatchingBuffer."Date Difference" = 0) and (TempMatchingDetailsBankStatementMatchingBuffer."Amount Difference" = 0) and (GetMaxTextScore(TempMatchingDetailsBankStatementMatchingBuffer) >= 95));
    end;
    ...
    begin
        ...
        TempMatchingDetailsBankStatementMatchingBuffer."UTR Nos" := BankAccReconciliationLine."UTR No";
        ...
        if BankAccReconciliationLine."Transaction Date" <> 0D then TransactionDateMatched := (TempMatchingDetailsBankStatementMatchingBuffer."Date Difference" = 0);
        ...
        if DocumentNoMatched or ExternalDocumentNoMatched or DescriptionMatched or AmountMatched then exit(true);
        exit(false);
    end;
    ...
    begin
        ...
        if not BankAccountLedgerEntry.FindSet() then exit;
        repeat
